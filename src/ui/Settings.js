...
const CROSSING_HINT = {
  ...
  bridge:
    'Diagonals connect, but crossings are resolved: the older link bridges over, the newer one is cut.',
...
const GOAL_HINT = {
  ...
  go:
    'Go on this tiling: groups without liberties (along shared edges) are captured, suicide is forbidden, ' +
...
const BORDER_HINT = {
  ...
  anchored:
    'Tiles whose group really reaches one of its owner’s sides get a painted border; it turns striped once the group reaches both.',
...
export class Settings {
  constructor({ dialogEl, tilings, themes, onConfig, onDisplay }) {
    ...
    const row = (label, control, extra) =>
      el('label', {}, el('span', { text: label }), control, extra);
    const checkRow = (control, label) =>
      el('label', { class: 'check' }, control, el('span', { text: label }));
    ...
    this.fields.size = el('input', {
      ...
      id: 'set-size',
      oninput: () => {
        this.sizeOut.textContent = this.fields.size.value;
      },
      onchange: cfgChanged,
    });
    ...
    const board = el(
      ...
      row(
        ...
        select(
          'tiling',
          tilings.map((t) => [t.id, `${t.name} (${t.vertexConfig})`]),
          cfgChanged
        )
      ),
      row(
        ...
        select(
          'outline',
          Object.entries(OUTLINES).map(([k, o]) => [k, `${o.label} — ${o.sides / 2} players`]),
          cfgChanged
        )
      ),
    ...
    const rules = el(
      ...
      row(
        ...
        select(
          'goal',
          GOALS.map((g) => [g, GOAL_LABEL[g] ?? g]),
          cfgChanged
        )
      ),
      this.goalHint,
      checkRow(
        check('pie', cfgChanged),
        'Pie rule — player 2 may swap colours instead of making their first move'
      )
    );
    ...
    const players = el(
      ...
      row(
        ...
        select(
          'clock',
          Object.entries(CLOCKS).map(([k, v]) => [k, clockText(k, v)]),
          cfgChanged
        )
      ),
      el('p', {
        class: 'hint',
        text:
          'Bots think in a background thread; difficulty is a time budget, not a handicap. ' +
          'Clocks are Fischer: main time per player plus an increment after each move. Running out of time loses.',
      }),
      el('p', {
        class: 'hint warn',
        text: 'Changing the board, the rules or the players starts a new game.',
      })
    );
    const display = el(
      'fieldset',
      ...
      row(
        ...
        select(
          'borders',
          BORDER_MODES.map((m) => [m, BORDER_LABEL[m] ?? m]),
          dispChanged
        )
      ),
      this.borderHint,
  ...
  readConfig() {
    ...
    return {
      tiling: f.tiling.value,
      outline: f.outline.value,
      size: Number(f.size.value),
      edgeMode: f.edgeMode.value,
      crossingMode: f.crossingMode.value,
      goal: f.goal.value,
  ...
  setConfig(cfg) {
    const f = this.fields;
    f.tiling.value = cfg.tiling;
    f.outline.value = cfg.outline;
    f.size.value = String(cfg.size);
    this.sizeOut.textContent = String(cfg.size);
    f.edgeMode.value = cfg.edgeMode;
    f.crossingMode.value = cfg.crossingMode;
    f.goal.value = cfg.goal;
    ...
    let key = Object.keys(this.clockValues).find(
      (k) => this.clockValues[k][0] === cfg.time && this.clockValues[k][1] === cfg.increment
    );
    if (!key) {
      // a custom clock from a URL: add it as an option so the select can show it
  ...
  readDisplay() {
    ...
    return {
      theme: f.theme.value,
      borders: f.borders.value,
      links: f.links.checked,
      animations: f.animations.checked,
    };
  }

  setDisplay(d) {
    const f = this.fields;
    f.theme.value = d.theme;
    f.borders.value = d.borders;
    f.links.checked = !!d.links;
    f.animations.checked = !!d.animations;
