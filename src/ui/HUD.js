...
export class HUD {
  constructor({ hudEl, toastEl, theme, onUndo, onSwap, onPass, onNew, onCopy, onSettings }) {
    ...
    this.passBtn = el('button', {
      text: 'Pass',
      title: 'Pass (P) — the game ends when everyone passes in turn',
      onclick: onPass,
    });
  ...
  update(game, config, board, tiling) {
    ...
    if (game.phase === 'won') {
      ...
      this.status.textContent =
        `${seatName(game.turn)} to move (move ${game.plies + 1})` +
