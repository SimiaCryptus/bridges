...
export class Board {
  constructor(topo, region, meta = {}) {
    ...
    this.vertices = topo.vertices.map((v) => ({
      id: v.id,
      p: v.p,
      cells: [],
      degree: 0,
    ...
    this.chords = [];
    this.arcs = this.sides.map((s) => ({
      id: s.id,
      side: s.id,
      cells: [],
      owner: s.id % this.players,
    }));
    this.scale = Math.sqrt(
      this.cells.reduce((s, c) => s + c.area, 0) / Math.max(1, this.cells.length)
    );
  ...
  get clean() {
    return this.vertices.every((v) => !v.interior || v.degree <= 3);
  }
  get maxDegree() {
    return this.vertices.reduce((m, v) => (v.interior ? Math.max(m, v.degree) : m), 0);
  }
  ...
  _buildVertices() {
    ...
    for (const v of this.vertices) {
      v.cells.sort(
        (x, y) => angle(sub(this.cells[x].centroid, v.p)) - angle(sub(this.cells[y].centroid, v.p))
      );
  ...
  _assignArcs() {
    for (const e of this.boundaryEdges) {
      ...
      const a = this.vertices[e.a].p,
        b = this.vertices[e.b].p;
  ...
  validate() {
    ...
    if (this.euler !== 1)
      console.warn(`[bridges] board is not a topological disc (V-E+F=${this.euler})`);
    ...
    while (stack.length) {
      const c = stack.pop();
      for (const n of this.cells[c].neighbors)
        if (!seen[n]) {
          seen[n] = 1;
          count++;
          stack.push(n);
