...
export function normalize(partial = {}, tilingIds = ['triangle']) {
  ...
  if (!tilingIds.includes(c.tiling))
    c.tiling = tilingIds.includes(DEFAULTS.tiling) ? DEFAULTS.tiling : tilingIds[0];
  ...
  c.bots = Array.from({ length: c.players }, (_, i) =>
    BOTS.includes(bots[i]) ? bots[i] : 'human'
  );
...
export function decodeMoves(s) {
  ...
  for (let i = 0; i + 1 < s.length; i += 2) {
    const hi = B64.indexOf(s[i]),
      lo = B64.indexOf(s[i + 1]);
    if (hi < 0 || lo < 0) break;
    const v = hi * 64 + lo;
