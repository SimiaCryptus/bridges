...
test('bundled tilings weld into a disc with one- or two-sided edges', () => {
  ...
  for (const [t, o] of combos) {
    ...
    for (const e of b.edges)
      assert.ok(e.cells.length >= 1 && e.cells.length <= 2, `${t}/${o} edge`);
...
test('the new tilings have the advertised vertex classes', () => {
  ...
  assert.ok(
    make({ tiling: 'truncTriHex', outline: 'square', size: 8 }).clean,
    '4.6.12 is trivalent'
  );
  ...
  for (const c of brick.cells)
    assert.equal(c.vertices.length, 6, 'bricks carry their edge midpoints');
  ...
  for (const c of cairo.cells.filter((c) => !c.boundary))
    assert.equal(c.vertices.length, 5, 'Cairo tiles are pentagons');
  const kinds = new Set(
    make({ tiling: 'truncTriHex', outline: 'square', size: 8 }).cells.map((c) => c.kind)
  );
