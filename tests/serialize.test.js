...
test('normalize repairs garbage', () => {
  const cfg = normalize(
    { tiling: 'nope', outline: 'blob', size: 999, edgeMode: 'x', crossingMode: 'y', goal: 'z' },
    tilingIds
  );
  ...
  assert.equal(
    normalize({ increment: 5 }, tilingIds).increment,
    0,
    'no increment without main time'
  );
});
